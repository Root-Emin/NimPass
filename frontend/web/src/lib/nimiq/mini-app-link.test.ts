...
describe('miniAppOpenerUrl', () => {
  it('keeps the package route, so Nimiq Pay opens on the same package', () => {
    expect(miniAppOpenerUrl('https://nimpass.app/packages/pkg_123')).toBe(
      'https://nimpay.app/miniapps/open/nimpass.app/packages/pkg_123',
    )
  })

  it('drops query and fragment, so no financial value can ride along', () => {
    // §85: a deep link may carry a package identifier, never an authoritative
    // recipient, price or quantity.
    expect(
      miniAppOpenerUrl('https://nimpass.app/packages/pkg_123?price=1&recipient=NQ_ATTACKER#x'),
    ).toBe('https://nimpay.app/miniapps/open/nimpass.app/packages/pkg_123')
  })
  ...
  it('offers no handoff from localhost, where the documented path is Custom URL', () => {
    expect(miniAppOpenerUrl('http://localhost:5173/packages/pkg_123')).toBeNull()
  })
  ...
  it('offers no handoff from a LAN address', () => {
    ...
    expect(miniAppOpenerUrl('http://192.168.1.105:5173/packages/pkg_123')).toBeNull()
  })
