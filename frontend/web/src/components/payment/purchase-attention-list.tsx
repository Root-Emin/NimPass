 ...
 * verified and whose package expired before activation has paid for something
 ...
 * package page or opening the app three days later.
 */
          ...
          <li key={purchase.purchaseIntentId}>
            <Card className="p-5">
              <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
                <p className="font-medium text-ink">{purchase.packageTitle}</p>
              ...
              <div className="mt-4">
                <Button asChild size="sm" variant="ghost">
                  {/* Back to the purchase, with recovery intact: the package
                      page reads this id from the URL and re-reads the
                      authoritative state rather than trusting anything here. */}
                  <Link to={`/packages/${purchase.packageId}?purchase=${purchase.purchaseIntentId}`}>
                    Open this purchase
                  </Link>
                </Button>
              </div>
