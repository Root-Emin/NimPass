import { CatalogPassCard, CatalogPassCardSkeleton } from '@/components/catalog/pass-card'
import { EmptyState, ErrorState } from '@/components/ui/states'
import type { PassListing } from '@/types/domain'
import type { ReactNode } from 'react'

/**
 * One place that decides how a pass list renders in each of its states, so
...
export function PassGrid({
    ...
    return (
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {Array.from({ length: skeletonCount }, (_, index) => (
          <CatalogPassCardSkeleton key={index} />
        ))}
      </div>
    )
  }
  ...
  return (
    <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
      {items.map((item) => (
        <CatalogPassCard key={item.id} item={item} />
      ))}
    </div>
  )
}
